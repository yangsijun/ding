...
public struct DispatchCommand: AsyncParsableCommand {
    public static let configuration = CommandConfiguration(
        commandName: "_dispatch",
        abstract: "Internal: handle AI tool hook events (reads context from stdin)",
        shouldDisplay: false
    )
    ...
    private func formatNotification(input: HookInput?, projectName: String) -> (title: String, body: String, status: NotificationPayload.Status) {
        ...
        let rawDir = input?.cwd ?? ProcessInfo.processInfo.environment["CLAUDE_PROJECT_DIR"] ?? ProcessInfo.processInfo.environment["GEMINI_PROJECT_DIR"] ?? ""
        let dirLine = shortenHome(rawDir)

        switch event {
        case "Stop", "AfterAgent":
            let (message, status) = summarizeCompletion(input: input)
    ...
    private func summarizeCompletion(input: HookInput?) -> (String, NotificationPayload.Status) {
        // 1. Explicit prompt field (Gemini, oh-my-opencode)
        if let prompt = input?.prompt, !prompt.isEmpty {
            return ("Done: \(truncate(prompt, maxLength: 80))", .success)
        }
        // 2. Claude Code: read last user message from transcript
        if let transcriptPath = input?.transcript_path {
            if let prompt = extractPromptFromTranscript(path: transcriptPath) {
                return ("Done: \(truncate(prompt, maxLength: 80))", .success)
            }
        }
        return ("Needs your attention", .warning)
    }

    private func extractPromptFromTranscript(path: String) -> String? {
        ...
        for line in content.components(separatedBy: "\n") where !line.isEmpty {
            guard let lineData = line.data(using: .utf8),
                  ...
                  !messageContent.isEmpty else { continue }
